export const meta = {
  ...
  title: 'Use clamp() instead of CSS Variables in Media Queries',
  summary:
    'Using clamp() instead of CSS Variables in Media Queries still makes a great responsive UX with a lot less code',
  tags: ['HTML', 'CSS'],
};
...
It is time to welcome the new clamp() function.

Instead of the media queries mess, we can now achieve the same responsivenes
with the clamp function.

It takes three parameters: a minimum value, a preferred value, and a maximum
allowed value.
...
export const cssVariables = `
  :root {
    ...
    // GAP VARIABLES
    --gap: clamp(var(--gap-unit-xs), 6vw, var(--gap-unit-xl));
    --gap-bottom-element: clamp(var(--gap-unit-sm), 8vw, var(--gap-unit-md));
    --gap-bottom-container: clamp(var(--gap-unit-lg), 12vw, var(--gap-unit-xl));
    ...
`;
```

And that is it 🎉 Now the code looks so much clean and easy to read.
